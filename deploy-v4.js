...
async function main() {
    console.log("🚀 开始部署 UltraSecureWheelGameV4 (优化奖励机制)...");
    ...
    try {
        // 部署合约
        console.log("\n🔨 正在部署合约...");
        const UltraSecureWheelGameV4 = await ethers.getContractFactory("UltraSecureWheelGameV4");
        
        const gameContract = await UltraSecureWheelGameV4.deploy(
            MAO_TOKEN_ADDRESS,
            PI_TOKEN_ADDRESS,
            ADMIN_ADDRESSES,
            TRUSTED_OWNER,
        ...
        // 保存部署信息到文件
        const fs = require('fs');
        fs.writeFileSync('V4_DEPLOYMENT_REPORT.json', JSON.stringify(deploymentReport, null, 2));
        console.log("\n💾 部署报告已保存到 V4_DEPLOYMENT_REPORT.json");
        
        console.log("\n🎉 部署完成! 合约已成功部署到 AlveyChain");
        console.log("🔗 合约地址:", contractAddress);
        console.log("\n🎯 优化特性:");
        console.log("- 35% 中奖率 (降低10%)");
        console.log("- 20% 利润保护");
        console.log("- 可持续奖励机制");
        console.log("- 约20% 预期利润率");
