...
contract UltraSecureWheelGameV6 is ReentrancyGuard, AccessControl, Pausable {
    // 角色定义
    ...
    uint256 public piGameCost = 1000 * 10**18; // 1000 PI (修正)
    ...
    // 每日奖励系统
    mapping(address => mapping(uint256 => bool)) public dailyRewards; // user => day => claimed
    ...
    function _initializeRewardLevels() private {
        ...
        rewardLevels.push(RewardLevel({
            probability: 1,
            maoReward: 5000 * 10**18,  // 5000 MAO (50倍)
            piReward: 50000 * 10**18,  // 50000 PI (50倍)
            name: "特等奖"
        }));
        ...
        rewardLevels.push(RewardLevel({
            probability: 5,
            maoReward: 1000 * 10**18,  // 1000 MAO (10倍)
            piReward: 10000 * 10**18,  // 10000 PI (10倍)
            name: "一等奖"
        }));
        ...
        rewardLevels.push(RewardLevel({
            probability: 20,
            maoReward: 300 * 10**18,   // 300 MAO (3倍)
            piReward: 3000 * 10**18,   // 3000 PI (3倍)
            name: "二等奖"
        }));
        ...
        rewardLevels.push(RewardLevel({
            probability: 50,
            maoReward: 150 * 10**18,   // 150 MAO (1.5倍)
            piReward: 1500 * 10**18,   // 1500 PI (1.5倍)
            name: "三等奖"
        }));
        ...
        rewardLevels.push(RewardLevel({
            probability: 174,
            maoReward: 30 * 10**18,    // 30 MAO (0.3倍)
            piReward: 300 * 10**18,    // 300 PI (0.3倍)
            name: "安慰奖"
        }));
    }
    ...
    function _calculateReward(uint256 winRate, uint8 tokenType, uint256 availableAmount) private view returns (uint256 rewardAmount, uint8 rewardLevel, bool isWin) {
        ...
        uint256 chance = randomSeed % 1000; // 使用1000作为基数提高精度
        
        if (chance < winRate * 10) { // 转换为1000基数
            isWin = true;
    ...
    function _updateUserState(address player, bool isWin) private {
        ...
        if (newLevel > level.level) {
            ...
            level.bonusRate = newLevel * 5; // 每级+5%奖励
